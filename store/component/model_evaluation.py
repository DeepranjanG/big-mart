...
from store.entity.config_entity import ModelEvaluationConfig
from store.entity.artifact_entity import DataIngestionArtifact,DataValidationArtifact,ModelTrainerArtifact,ModelEvaluationArtifact,DataTransformationArtifact
from store.constant import *
import numpy as np
import os
import sys
from store.util.util import write_yaml_file, read_yaml_file, load_object,load_data
from store.entity.model_factory import evaluate_regression_model
import pandas as pd 
...
class ModelEvaluation:
    ...
    def encoding(self,dataframe):
        ...
        # Changing Values of item Identifier because store of item identifier are more so giving high value
        Item_Identifier = {'DR': 1, 'FD': 3, 'NC': 2}
        # Changing Values of Item_Fat_Content because store of Regular are more so giving high value
        Item_Fat_Content = {'Low Fat': 1, 'Non Edible': 0, 'Regular': 2}
        # Changing Values of outlet size because store in medium size outlets are more so giving high value
